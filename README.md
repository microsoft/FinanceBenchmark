...
| [ERP QA setup](docs/erp_qa_setup.md) | Dynamics 365 setup requirements for ERP QA |
...
ERP QA requires a Dynamics 365 Finance sandbox provisioned with demo data. Follow the Microsoft Learn provisioning tutorial first:
...
Then follow the benchmark docs in order:

1. [Importing Synthetic Data](docs/importing_synthetic_data_into_dynamics_365_finance.md)
2. [Setting Up the ERP MCP Server](docs/setting_up_mcp_for_erp_qa.md)

Once connected, set `shared.mcp_server_label` in `config.yaml` to your MCP server entry and run normally. This path requires a bearer token (`uv run refresh_erp_token.py`).
