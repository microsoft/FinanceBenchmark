...
For full credential setup, account switching, and troubleshooting: **[authentication.md](authentication.md)**.
...
Check that `shared.mcp_server_label` in `config.yaml` matches a key in `.mcp.json`. See [authentication.md](authentication.md) for switching environments.

### Missing API key
**Symptom:** evaluation or inference fails immediately with an auth error.

Check your `.env` file. See [authentication.md](authentication.md) for all required variables.

### Azure CLI / wrong account
**Symptom:** `refresh_erp_token.py` fails, or infra commands fail.

See [authentication.md](authentication.md) — two different Azure accounts are used for different purposes.
