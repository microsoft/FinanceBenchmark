...
Provider-specific logic lives in `scripts/inference/_claude.py` and `scripts/inference/_openai.py`. Each exports:
...
To add a new provider, implement these two functions and register the new provider in `inference.py`.
