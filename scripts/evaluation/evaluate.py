...
One judge call per tag per question. Plugin-aware routing:
  ...
  - finance_qa / citations    → standard LLM judge (assertion credits named data providers, not just markdown hyperlinks)
  - erp_qa / citations        → standard LLM judge (same as finance_qa / business_brief)
...
JUDGE_CLIENT           = _cfg["evaluation"].get("judge_client", "openai")
_CFG_RUN_ID            = _cfg["shared"].get("run_id") or None

TAG_EVALUATOR_INSTRUCTIONS          = _cfg["evaluation"]["tag_evaluator_instructions"].strip()
ACCURACY_EVALUATOR_INSTRUCTIONS     = _cfg["evaluation"].get("accuracy_evaluator_instructions", "").strip()
...
_accuracy_instructions           = (TAG_EVALUATOR_INSTRUCTIONS + "\n\n" + ACCURACY_EVALUATOR_INSTRUCTIONS
                                    if ACCURACY_EVALUATOR_INSTRUCTIONS else TAG_EVALUATOR_INSTRUCTIONS)
accuracy_tag_assertion_evaluator = TagAssertionEvaluator.with_instructions(_accuracy_instructions)
...
def make_skipped_scores(eval_entry: dict | None, reason: str, plugin: str = "") -> dict:
    """Build zero-filled score fields for a skipped (no-answer) entry."""
    tag_scores: dict = {}
    assertion_scores: dict = {}
    tag_reasoning: dict = {}
    rationale = f"Scored 0: no response — {reason}"

    for tag_entry in (eval_entry or {}).get("tags", []):
        tag = tag_entry["tag"]
        assertions = tag_entry.get("assertions", [])
        if not assertions:
            continue
        assertion_scores[tag] = {a["text"]: 0.0 for a in assertions}
        tag_scores[tag] = 0.0
        tag_reasoning[tag] = rationale

    overall = 0.0 if tag_scores else None
    result = {
    ...
    if plugin == "business_brief":
        result["section_assertion_scores"] = {}
        result["section_reasoning"] = {}
    return result
...
def get_sources(answer_entry: dict) -> str:
    ...
    # web_search outputs are url+title citation lists, not page content — useless for
    # groundedness. Skip them so entries with only web_search calls are treated as having
    # no sources (groundedness skipped) rather than scored against citation metadata.
    _WEB_SEARCH_TOOLS = {"web_search", "WebSearch"}

    for tc in answer_entry.get("tool_calls", []):
        if tc.get("success") and tc.get("output") and tc.get("tool") not in _WEB_SEARCH_TOOLS:
...
def load_sections_rubric(rubric_path: str) -> dict:
    """Load sections_rubric.yml — {section: {dimension: [assertion_texts]}}."""
...
def evaluate_tag(
    ...
    accuracy_judge: dspy.Module | None = None,
) -> dict:
    """Evaluate a single tag. Returns a result dict with type in {scored, skipped}."""
    tag = tag_entry["tag"]

    # skip tags that have no assertions — no LLM call, no score
    if not tag_entry.get("assertions"):
        return {"type": "skipped", "reason": f"no assertions defined for tag '{tag}'"}
    ...
    # accuracy: use dedicated judge with accuracy-specific partial-credit instructions
    if tag == "accuracy" and accuracy_judge is not None:
        active_judge = accuracy_judge
    else:
        active_judge = judge
...
def evaluate_entry(
    ...
    bb_sections_rubric: dict | None = None,
) -> dict:
    ...
    for tag_entry in tags:
        ...
        result = evaluate_tag(
            ...
            judge, groundedness_judge, accuracy_judge,
        )
    ...
    if plugin == "business_brief" and bb_sections_rubric:
        sec = evaluate_bb_sections(
            question, answer, bb_sections_rubric,
    ...
    result: dict = {
        ...
        "inference_time_secs":  answer_entry.get("inference_time_secs"),
        "overall_score":        overall,
        "tag_scores":           tag_scores,
...
def main():
    ...
    parser.add_argument("--judge-model",      default=None, metavar="NAME",
                        help="Override config evaluation.judge_deployment (model name or deployment ID)")
    parser.add_argument("--no-cache",         action="store_true",
                        help="Documents that DSPy LM caching is disabled (no additional effect)")
    parser.add_argument("--tags",    metavar="TAG",    action="append", default=None,
                        help="Only evaluate these tags; repeatable (e.g. --tags accuracy --tags citations)")
    parser.add_argument("--plugins", metavar="PLUGIN", action="append", default=None,
                        help="Only evaluate these plugins; repeatable. Aliases: fqa, eqa, bb")
    parser.add_argument("--sections", action="store_true",
                        help="Enable business brief section-by-section evaluation (default: off)")
    args = parser.parse_args()

    _PLUGIN_ALIASES = {"fqa": "finance_qa", "eqa": "erp_qa", "bb": "business_brief"}

    tag_filter    = set(args.tags) if args.tags else None
    plugin_filter = {_PLUGIN_ALIASES.get(p, p) for p in args.plugins} if args.plugins else None
    ...
    from scripts.tracking.db import _extract_slug_from_filename, generate_unique_slug
    ...
    if tag_filter:
        print(f"  Tag filter:       {', '.join(sorted(tag_filter))}")
    if plugin_filter:
        print(f"  Plugin filter:    {', '.join(sorted(plugin_filter))}")
    ...
    print(f"\nInitializing judge LM: {judge_deployment} (via {JUDGE_CLIENT})")
    if JUDGE_CLIENT == "openai":
        from scripts.evaluation.openai_client import OpenAIJudgeLM
        judge_lm = OpenAIJudgeLM(model=judge_deployment, reasoning_effort=JUDGE_REASONING_EFFORT)
        if not judge_lm.api_key:
            raise SystemExit("OpenAI auth failed — check OPENAI_API_KEY in .env.")
    else:
        raise SystemExit(f"Unknown judge_client: {JUDGE_CLIENT!r} — expected 'openai'")
    ...
    accuracy_judge     = dspy.Predict(accuracy_tag_assertion_evaluator) if ACCURACY_EVALUATOR_INSTRUCTIONS else judge

    # ── Business Brief section eval setup ────────────────────────────────────
    bb_sections_rubric = {}
    section_extractor  = None
    section_evaluator  = None
    if args.sections:
        sections_rubric_path = None
        for ds in _cfg.get("data_prep", {}).get("datasets", []):
            if ds.get("plugin") == "business_brief" and ds.get("sections_rubric"):
                sections_rubric_path = ds["sections_rubric"]
                break
        if sections_rubric_path:
            rubric_abs = Path(__file__).parent.parent.parent / sections_rubric_path
            if rubric_abs.exists():
                bb_sections_rubric = load_sections_rubric(str(rubric_abs))
            else:
                print(f"  [warn] sections_rubric not found at {rubric_abs}; --sections produces empty scores")
        else:
            print("  [warn] --sections passed but no sections_rubric in config; skipping section eval")
    ...
    for entry in answers:
        ...
        plugin     = entry.get("plugin") or (eval_entry.get("plugin", "") if eval_entry else "")

        # Plugin filter: skip entirely — entry won't appear in output at all
        ...
        if entry.get("error") or not answer:
            ...
            skipped = {
                "question":            question,
                "plugin":              plugin,
                "inference_time_secs": entry.get("inference_time_secs"),
                "skipped":             True,
                "reason":              reason,
            }
            skipped.update(make_skipped_scores(eval_entry, reason, plugin))
        ...
        if not eval_entry:
            ...
            skipped.update(make_skipped_scores(None, reason, plugin))
            skipped_results.append(skipped)
            continue

        tags = eval_entry.get("tags") or []
        if tag_filter:
            tags = [t for t in tags if t.get("tag") in tag_filter]
            if not tags:
                continue  # no relevant tags for this entry, skip entirely

        # Resume: skip already-evaluated entries unless tag filter forces re-evaluation
        if question in existing and not tag_filter:
            continue

        to_evaluate.append((question, answer, plugin, tags, entry))

    filter_note = ""
    if tag_filter or plugin_filter:
        parts = []
        if tag_filter:
            parts.append(f"tags={sorted(tag_filter)}")
        if plugin_filter:
            parts.append(f"plugins={sorted(plugin_filter)}")
        filter_note = f"  [filtered: {', '.join(parts)}]"
    print(f"\nEvaluating {len(to_evaluate)} answers "
          f"({len(existing)} already done, {len(skipped_results)} skipped){filter_note}...\n")
    ...
    def _worker(item):
        ...
        return evaluate_entry(
            ...
            bb_sections_rubric=bb_sections_rubric,
        )
    ...
    config_snapshot = {
        ...
        "eval_file":                           args.eval_file,
        "tag_evaluator_instructions":          TAG_EVALUATOR_INSTRUCTIONS,
        "accuracy_evaluator_instructions":     ACCURACY_EVALUATOR_INSTRUCTIONS,
        "groundedness_evaluator_instructions": GROUNDEDNESS_EVALUATOR_INSTRUCTIONS,
    }
    ...
    def _compute_summary(results: list) -> dict:
        ...
        # Include skipped entries (which carry zero scores) in all aggregate metrics
        scored_r      = [r for r in results if r.get("overall_score") is not None]
        overall_sc    = [r["overall_score"] for r in scored_r]
    ...
    def _merge_into_existing(result: dict) -> dict:
        """When tag_filter is active, merge new tag scores into the previously evaluated entry."""
        q = result.get("question", "")
        if not (tag_filter and q in existing):
            return result
        merged = {**existing[q]}
        merged["tag_scores"]       = {**merged.get("tag_scores", {}),       **result["tag_scores"]}
        merged["assertion_scores"] = {**merged.get("assertion_scores", {}), **result["assertion_scores"]}
        merged["tag_reasoning"]    = {**merged.get("tag_reasoning", {}),    **result["tag_reasoning"]}
        all_scores = list(merged["tag_scores"].values())
        merged["overall_score"] = round(sum(all_scores) / len(all_scores), 3) if all_scores else None
        old_usage = merged.get("token_usage", {})
        new_usage = result.get("token_usage", {})
        merged["token_usage"] = {
            k: old_usage.get(k, 0) + new_usage.get(k, 0)
            for k in ("input_tokens", "output_tokens", "total_tokens")
        }
        existing[q] = merged  # update in-place so _flush picks up the merged version
        return merged
    ...
    with ThreadPoolExecutor(max_workers=args.threads) as executor:
        ...
        for future in tqdm(as_completed(futures), total=len(futures), desc="Evaluating"):
            try:
                new_results.append(_merge_into_existing(future.result()))
            except Exception as exc:
                _q, _a, _plugin, _tags, _entry = futures[future]
                _eval_entry = eval_index.get(_q.strip())
                _reason = f"evaluation error: {exc}"
                _skipped = {
                    "question":            _q,
                    "plugin":              _plugin,
                    "inference_time_secs": _entry.get("inference_time_secs"),
                    "skipped":             True,
                    "reason":              _reason,
                }
                _skipped.update(make_skipped_scores(_eval_entry, _reason, _plugin))
                new_results.append(_skipped)
            _flush(new_results)
    ...
    # Compute final summary — include skipped entries (zero scores) in all aggregates
    evaluated      = [r for r in all_results if not r.get("skipped")]
    skipped        = [r for r in all_results if r.get("skipped")]
    scored         = [r for r in all_results if r.get("overall_score") is not None]
