...
Python API
----------
    from scripts.analysis.query_runs import query_runs
...
---
    ...
    uv run scripts/analysis/query_runs.py list --inf-run-id octopus buffalo
    uv run scripts/analysis/query_runs.py list --eval-run-id tuna marmot

    # Export flat per-(run, question) table
    uv run scripts/analysis/query_runs.py export [--provider claude] [--output out.csv]

    # Preview runs to delete (dry-run)
    uv run scripts/analysis/query_runs.py delete --provider claude
    uv run scripts/analysis/query_runs.py delete --id 5

    # Actually delete (add --yes; optionally remove result files from disk too)
    uv run scripts/analysis/query_runs.py delete --provider claude --yes
    uv run scripts/analysis/query_runs.py delete --id 5 --yes --files
"""
...
def _build_eval_index(results: list[dict]) -> dict[str, dict]:
    return {r["question"]: r for r in results if r.get("question") and not r.get("skipped")}
