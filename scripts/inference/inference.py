...
Single entry point for Finance Copilot Benchmark inference.
...
def _check_azure_account() -> None:
    """Exit with an error if logged into the demo tenant account.
    ...
    *,
    ...
    async def process_and_save(row: dict, semaphore: asyncio.Semaphore, index: int, total: int) -> dict:
        ...
        async with write_lock:
            ...
            _write_output(output_file, results, metadata)
        return result
    ...
    for i, r in enumerate(raw_results):
        if isinstance(r, Exception):
            row = pending_rows[i]
            print(f"  ERROR on question {len(done_results) + i + 1}: {r}")
...
async def main(
    ...
    if output_file is None:
        from scripts.analysis.runs_db import generate_unique_slug
        ...
        from scripts.analysis.runs_db import _extract_slug_from_filename
