"""
run_benchmark.py - Finance Copilot Benchmark pipeline entry point
...
def _try_compute_hash(config: dict, provider: str, args: argparse.Namespace) -> str | None:
    ...
    try:
        if provider == "sydney":
            from scripts.data_prep.prepare_sydney_answers import compute_sydney_config_hash  # type: ignore
            sydney_cfg = config.get("data_prep", {}).get("sydney", {})
            return compute_sydney_config_hash(sydney_cfg)
...
def _open_db():
    ...
    try:
        from scripts.analysis.runs_db import init_db  # type: ignore
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Finance Copilot Benchmark pipeline: data prep -> inference -> evaluation."
    )
    parser.add_argument(
        "--provider",
        choices=["claude", "openai", "sydney", "all"],
        default="all",
    ...
    # Load .env and check for ERP token (not needed for sydney-only runs)
    load_dotenv()
    if args.provider != "sydney" and not os.getenv("ERP_MCP_TOKEN"):
        print(
    ...
    if not args.skip_inference:
        for provider in providers:
            ...
            if not args.no_reuse and db_conn is not None and config_hash:
                from scripts.analysis.runs_db import find_reusable_inference  # type: ignore
            ...
            if reused_file is None:
                if provider == "sydney":
                    cmd = ["uv", "run", "scripts/data_prep/prepare_sydney_answers.py"]
                else:
    ...
    if not args.skip_eval:
        for provider in providers:
            ...
            if db_conn is not None and config_hash:
                from scripts.analysis.runs_db import find_latest_inference  # type: ignore
            ...
            if not args.no_reuse and db_conn is not None and config_hash:
                ...
                from scripts.analysis.runs_db import find_reusable_eval  # type: ignore
                _eval_snapshot = {
